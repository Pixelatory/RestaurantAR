            ...
            this.props.categories.map((item) => (
              <Fragment key={item.dish_categories_id}>
                <Text style={styles.categoryText}>
                  {item.dish_categories_name}
                </Text>
                {this.filterDishesOnCategory(
                  this.filterList(this.props.dishes),
                  item.dish_categories_id,
                ).map((listItem) => (
                  <TouchableHighlight
                    activeOpacity={0.9}
                    underlayColor="#F09133"
                    onPress={() => {
                      this.props.fullItem(listItem.dish_id);
                    }}
                    key={listItem.dish_name}>
                    <Dish
                      name={listItem.dish_name}
                      info={listItem.dish_description}
                      cost={listItem.dish_price}
                    />
                  </TouchableHighlight>
                ))}
              </Fragment>
